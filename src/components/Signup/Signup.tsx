  ...
  return (
    <div className="wrapper w-full h-screen bg-white">
      <div className="content h-[570px] flex flex-col justify-between">
        ...
        <div >
          <button className='flex justify-center items-center w-full gap-x-2 bg-white hover:bg-slate-100 rounded-md p-2 mt-4' onClick={() => { signInWithGoogle(router) }}>
            <Image src={assets.google} alt='Google_icon' id='icon'></Image>Signin with Google
          </button>
        </div>
        <Link href="/login" className='mt-3'>Already have an account</Link>
      </div>
    </div>
  );
};
